"""Servicio de integración con Binance Pay (Merchant API).
...
def _build_signature(timestamp: str, nonce: str, payload: str) -> str:
    """Build HMAC-SHA512 signature for Binance Pay API."""
    import hmac
    message = f"{timestamp}\n{nonce}\n{payload}\n"
    return hmac.new(
...
def _get_headers(payload: str) -> dict:
    """Build headers with Binance Pay authentication."""
...
def create_payment_order(
    ...
    Returns dict with checkout URL / QR data, or None on error.
    """
    ...
    try:
        with httpx.Client(timeout=15) as client:
            ...
            if data.get("status") == "SUCCESS":
                ...
                return {
                    "status": order_data.get("status"),  # PAID, PENDING, EXPIRED, etc.
